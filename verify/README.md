...
| P06 | `int = int / int` 合法性 | `Any2EnforceProbes/P06_IntDivToInt.c` | 编译过→输出 3/4；失败→需 (int) 强转 | ✅ **定案：可编译，值 = 3（向零截断）**；`//` 映射 `Math.Floor` 对负数语义仍正确 |
...
| P09 | map.Contains / Get 回退 | `Any2EnforceProbes/P09_MapContains.c` | A 或 B 哪个编译过 | ✅ **定案：map.Contains 存在**（`x in dict` → `m.Contains(x)` 已实现） |
